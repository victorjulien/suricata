 ...
 *
 * \param skip_fw_hook bool to indicate firewall rules for state `skip_before_progress` should be skipped.
 * \param skip_before_progress progress value to skip rules before. Only used if `skip_fw_hook` is set.
 * \param last_for_progress[out] set to true if this is the last rule for a progress value
 * \param fw_next_progress_missing[out] set to true if the next fw rule does not target the next progress value, or there is no fw rule for that value.
 *
 ...
 */
static inline bool ApplyAcceptToPacket(const uint64_t total_txs, const DetectTransaction *tx, const Signature *s)
{
