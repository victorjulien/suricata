        ...
        void (*FreeFunc)(void *pectx), const char *name)
{
    ...
    e->name = name;
    e->gid = PrefilterStoreGetId(de_ctx, e->name, e->Free);
    /* special case handling for non-prefilter engine: it is stateless in the sense
     * that it doesn't update any state. */
    e->is_stateless = !(strcmp(name, "tx::non_pf"));
    return 0;
}
...
struct PrefilterNonPFDataSig {
    uint32_t sid : 30;
    uint32_t type : 2; // 0 alproto 1 dport 2 dsize
    uint16_t value;
    union {
        struct {
...
{
    const uint16_t alproto = f->alproto;
...
/* TODO the old code checked alproto and mask */
...
int PrefilterSetupRuleGroup(DetectEngineCtx *de_ctx, SigGroupHead *sgh)
{
    ...
    if (sgh->init->tx_engines != NULL) {
        ...
        for (el = sgh->init->tx_engines ; el != NULL; el = el->next) {
            ...
            el->pectx = NULL; // e now owns the ctx
            e->gid = el->gid;
            e->is_stateless = el->is_stateless;
            e++;
        }
