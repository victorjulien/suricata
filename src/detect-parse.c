...
static void PolicyToString(const struct DetectFirewallPolicy *p, char *out, size_t out_size)
{
        ...
        struct DetectFirewallPolicy *pol)
{
    ...
    char msg[256];
    snprintf(
            msg, sizeof(msg), "default app policy %s for %s:%s", policy_string, app_name, hookname);
    s->msg = SCStrdup(msg);
    if (s->msg == NULL) {
        SCFree(s);
        return -1;
