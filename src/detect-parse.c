 ...
 * \param dash_key Policy key as written in the config (dash form)
 *
 * \retval true If the key is a non-state policy key or matches a
 *              current state name in either direction
 */
static bool FwPolicyStateKeyIsKnown(AppProto a, const char *sub_state_name, const char *dash_key)
{
    /* non-state keys of the policy config */
    if (strcmp(dash_key, "default-policy") == 0 || strcmp(dash_key, "request-started") == 0 ||
            strcmp(dash_key, "response-started") == 0 ||
            strcmp(dash_key, "request-complete") == 0 ||
            strcmp(dash_key, "response-complete") == 0) {
        return true;
    }
        ...
        const char *prefix, AppProto a, const char *sub_state_name)
{
    ...
    char path[FW_POLICY_YAML_PATH_NAME_MAX * 2 + 16];
    char sub[FW_POLICY_YAML_PATH_NAME_MAX + 1] = "";
    if (sub_state_name != NULL) {
        sub[0] = '.';
        FirewallHookNameConvertUnderscoreToDash(sub_state_name, sub + 1, sizeof(sub) - 1);
    }
    snprintf(path, sizeof(path), "%s.app.%s%s", prefix, proto, sub);
    ...
    TAILQ_FOREACH (c, &node->head, next) {
        ...
        if (FwPolicyStateKeyIsKnown(a, sub_state_name, c->name)) {
            continue;
        }
...
int DetectFirewallLoadDefaultPolicies(DetectEngineCtx *de_ctx)
{
    ...
    if (known_wrong_keys) {
        ...
        FatalErrorOnInit("firewall policy: %d key(s) match no state and are "
                         "known to be renamed - aborting init",
                known_wrong_keys);
    }
    return 0;
}
