...
static inline int TlsDecodeHSCertificateFingerprint(
        char **out, const uint8_t *input, uint32_t cert_len)
{
    *out = SCCalloc(1, SHA1_STRING_LENGTH);
    if (*out == NULL)
        return -1;

    uint8_t hash[SC_SHA1_LEN];
    if (SCSha1HashBuffer(input, cert_len, hash, sizeof(hash)) == 1) {
        SCToHex_sep((uint8_t *)*out, SHA1_STRING_LENGTH, ':', hash, SC_SHA1_LEN);
    }
    return 0;
}
        ...
        bool *extract_event_sent)
{
    ...
    if (certn == 0 && connp->cert0_subject == NULL && connp->cert0_issuerdn == NULL &&
        ...
        rc = TlsDecodeHSCertificateFingerprint(&st_fingerprint, input, cert_len);
        if (rc != 0) {
        ...
        connp->cert0_fingerprint = st_fingerprint;
...
error:
    ...
    /* the certificate failed before the publish block: nothing was
     * published, release the stashed extraction so the next certificate
     * can fill cert0 */
    if (st_subject != NULL)
    ...
    if (st_fingerprint != NULL)
        SCFree(st_fingerprint);
    if (st_sans != NULL) {
        for (uint16_t i = 0; i < st_sans_num; i++)
                                   ...
                                   uint32_t input_len, uint8_t direction)
{
    ...
    switch (ssl_state->curr_connp->handshake_type) {
        ...
        case SSLV3_HS_CLIENT_KEY_EXCHANGE:
            ...
            /* a client flight message: the server flight is over, no
             * CertificateRequest can still arrive */
            ssl_state->current_flags |= SSL_AL_FLAG_SERVER_FLIGHT_DECIDED;
            break;

        case SSLV3_HS_CERTIFICATE:
            if (!direction) {
                /* the client certificate: its own flight (and so the
                 * server's CertificateRequest decision) is past */
                ssl_state->current_flags |= SSL_AL_FLAG_SERVER_FLIGHT_DECIDED;
            }
        ...
        case SSLV3_HS_CERTIFICATE_REQUEST:
            if (direction) {
                ssl_state->current_flags =
                        SSL_AL_FLAG_NEED_CLIENT_CERT | SSL_AL_FLAG_SERVER_FLIGHT_DECIDED;
            }
            break;
        case SSLV3_HS_CERTIFICATE_VERIFY:
        case SSLV3_HS_CERTIFICATE_URL:
 ...
 * parses, never during one. The client certificate decision must also
 * be settled: the standard flight order puts the CertificateRequest
 * after the server Certificate, so server content readiness alone can
 * precede it - logging there would miss the client certificate
 * content and the decoder events a client flight may still raise (the
 * TLS tx logs once; base logged at server flight end, where both were
 * included). The decision is settled by the NEED_CLIENT_CERT flag, by
 * any message at or past the CertificateRequest position (server
 * flight: CertificateRequest, ServerHelloDone, Finished; client
 * flight: its Certificate or ClientKeyExchange - flag
 * SERVER_FLIGHT_DECIDED), by a change cipher spec record, or by the
 * handshake ending at close. */
...
bool SSLV3TxLogReady(void *tx)
{
    ...
    } else if ((ssl_state->flags &
                       (SSL_AL_FLAG_SERVER_FLIGHT_DECIDED | SSL_AL_FLAG_CHANGE_CIPHER_SPEC)) == 0 &&
               ssl_state->server_state < TLS_STATE_SERVER_FINISHED) {
        /* a CertificateRequest may still arrive in this flight */
        return false;
    }
    return true;
}
