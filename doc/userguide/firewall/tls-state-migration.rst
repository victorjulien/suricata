...
when the unit completes. The phase data (SNI, certificates) is
filled only once the message has been fully parsed; rules evaluated
while it is still arriving (a fragmented message) see no data at
all, so their content keywords cannot match. The state names used
in rules
...
.. list-table::
   ...
   * - ``client_cert_done``
     - ``client_cert``
     - entered when the client hello completes, before any certificate
       byte: the records up to the completing certificate message carry
       no certificate data - add a plain ``accept:hook tls:client_cert``
       (or a drop-based cert check plus that accept) for them
   ...
   * - ``server_cert_done``
     - ``server_cert``
     - entered when the server hello completes, before any certificate
       byte: the records up to the completing certificate message carry
       no certificate data - add a plain ``accept:hook tls:server_cert``
       (or a drop-based cert check plus that accept) for them
   * - ``server_hello_done``
     - ``server_data``
...
A plain accept at ``client_cert`` or ``server_cert`` - including the
pairing of a drop-based cert check with that accept - also decides the
record that completes the hello, and with it a ClientHello that
carries no SNI at all: the SNI buffer is only filled once the message
is fully parsed, so no ``tls.sni`` rule (including a negated one) can
match it. The base release dropped that hello at the
``client_hello_done`` milestone through the implicit default policy; a
certificate-state accept accepts it. Keep the certificate state free
of an accept to preserve the base outcome - the implicit default
policy decides it, identically to the milestone - or add an explicit
rule for the SNI-less case if it must be accepted. The SNI example in
the firewall user guide and the upgrade chapter carry the same
warning.
...
- ``client_hello`` names the ClientHello being parsed: it is entered
  ...
  ``client_hello``: the hand-over to the certificate phase is
  suppressed, but the fields the parser already filled (version, session
  id, an SNI extension parsed before the failure) stay in the connp and
  remain matchable - the phase data may be incomplete rather than
  absent, as in the previous model. ``server_hello`` behaves
...
- The TLS event carrying the ``ja3``/``ja3s`` fingerprint fields is a
  ...
  for the client certificate. While the server flight has not yet
  revealed whether a client certificate is required - the
  CertificateRequest normally rides after the server Certificate - the
  event waits for that to be decided (a message at or past the
  CertificateRequest position, a change cipher spec, or the close of
  the handshake), so an mTLS client certificate and its decode
  failures are still included, as they were when the old logger flushed
  at server flight end. A session that never reaches that point
...
2. **Firewall config keys.** Config keys use the **dash** form
   ...
   state is an error naming the replacement (fatal when
   ``init-failure-fatal`` is set), other unknown keys are a warning
   and are ignored.
